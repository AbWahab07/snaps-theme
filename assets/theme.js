...
(function($){
  $(document).ready(function(){
    ...
    if($('.shopify-section').hasClass('featured-products')){
      if ($(window).width() > 750) {
        $('.featured-products').each(function() {
          var heightFeaturedProducts = $(this).height();
          if(!$(this).find('.image .hero__btn').hasClass('second')){
            $(this).find('.image .hero').css('height', heightFeaturedProducts - 30);
          }else{
            $(this).find('.image .hero').css('height', heightFeaturedProducts);
